...
void Ichigo::Internal::set_mouse_pos(i32 x, i32 y) {
    POINT p = {x, y};
    if (ClientToScreen(window_handle, &p)) {
        SetCursorPos(p.x, p.y);
    }
}

Vec2<i32> Ichigo::Internal::get_window_pos() {
    RECT r;
...
i32 WinMain(HINSTANCE instance, [[maybe_unused]] HINSTANCE prev_instance, [[maybe_unused]] LPSTR cmd_line, [[maybe_unused]] i32 show_cmd) {
    ...
    window_handle = CreateWindowEx(0, window_class.lpszClassName, L"MI MI ZU HAPPY", WS_OVERLAPPEDWINDOW | WS_VISIBLE, CW_USEDEFAULT, CW_USEDEFAULT, Ichigo::Internal::window_width, Ichigo::Internal::window_height, nullptr,
...
#define GET_ADDR_OF_OPENGL_FUNCTION(FUNC_NAME)                                                            \
    ...
    GET_ADDR_OF_OPENGL_FUNCTION(glDepthFunc);
